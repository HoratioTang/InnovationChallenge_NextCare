...
def fusion_agent(state: AgentState) -> dict:
    """
    Decision fusion agent that merges scalar probabilities from different branches.
    Returns a partial dict (LangGraph convention for parallel-safe nodes).
    """
    ...
    if state.acoustic_result is None or state.semantic_result is None:
        return {
            "errors": ["[fusion_agent] Missing classification results from acoustic or semantic branch."],
            "messages": ["[fusion_agent] Error: Incomplete branch data."],
        }

    # Instantiate handler with weights from config
    handler = DecisionFusionHandler(
        w_acoustic=FUSION_WEIGHTS["acoustic"],
    ...
    try:
        ...
        return {
            "fusion_result": results["final_score"],
            "messages": [
        ...
        return {
            "errors": [f"[fusion_agent] {e}"],
            "messages": [f"[fusion_agent] Exception occurred: {e}"],
        }
